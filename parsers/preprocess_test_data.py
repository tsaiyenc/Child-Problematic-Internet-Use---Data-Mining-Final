...
def preprocess_data(input_file, dictionary_file, output_file):
    ...
    # Check for 'sii' column (Difference from train script)
    if 'sii' in df.columns:
        ...
        print("'sii' column not found (Test set detected). Skipping dropna on 'sii'.")
        df_cleaned = df.copy()
    ...
    for col in df_cleaned.columns:
        ...
        # Even if not missing in this specific file, we apply logic if needed, but here we check isna
...
if __name__ == "__main__":
    ...
    input_path = os.path.join(base_dir, "data", "test.csv")
    dict_path = os.path.join(base_dir, "data", "data_dictionary.csv")
    output_path = os.path.join(base_dir, "data", "test_cleaned_imputed.csv")
